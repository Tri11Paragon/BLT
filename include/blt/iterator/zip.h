...
namespace blt
{
    namespace iterator
    {
        ...
        struct zip_wrapper : base_wrapper<zip_wrapper<Iter...>>
        {
            ...
            zip_wrapper& operator--()
            {
                static_assert(meta::is_bidirectional_or_better_category_v<iterator_category>,
                              "Iterator must be bidirectional or better!");
                std::apply([](auto&... i) { ((--i), ...); }, this->iter);
                return *this;
            }

            friend zip_wrapper operator+(const zip_wrapper& a, ptrdiff_t n)
            {
                static_assert(meta::is_random_access_iterator_category_v<iterator_category>,
            ...
            friend ptrdiff_t operator-(const zip_wrapper& a, const zip_wrapper& b)
            {
        ...
        protected:
            ...
            static ptrdiff_t sub(const zip_wrapper& a, const zip_wrapper& b,
                                      std::integer_sequence<T, n...>)
            {
                ptrdiff_t min = std::numeric_limits<ptrdiff_t>::max();
    ...
    public:
        ...
        explicit zip_iterator_container(iterator::iterator_pair<Iter>... iterator_pairs) :
            iterator::iterator_container<iterator::zip_wrapper<Iter...>>(
                blt::iterator::zip_wrapper<Iter...>{std::move(iterator_pairs.begin)...},
    ...
    static auto in_pairs(const T& container1, const G& container2)
    {
        return zip_iterator_container{
            iterator::iterator_pair{container1.begin(), container1.end()},
            iterator::iterator_pair{container2.begin(), container2.end()}
        };
    }

    template <typename T, typename G>
    static auto in_pairs(T& container1, G& container2)
    {
        return zip_iterator_container{
            iterator::iterator_pair{container1.begin(), container1.end()},
            iterator::iterator_pair{container2.begin(), container2.end()}
    ...
    template <typename T, typename G, size_t size>
    static auto in_pairs(const T (&container1)[size], const G (&container2)[size])
    {
        return zip_iterator_container{
            iterator::iterator_pair{&container1[0], &container1[size]},
            iterator::iterator_pair{&container2[0], &container2[size]}
        };
    }

    template <typename T, typename G, size_t size>
    static auto in_pairs(T (&container1)[size], G (&container2)[size])
    {
        return zip_iterator_container{
            iterator::iterator_pair{&container1[0], &container1[size]},
            iterator::iterator_pair{&container2[0], &container2[size]}
