...
namespace blt::meta
{
    ...
    struct filter_tuple<Func>
    {
        using type = std::tuple<>;
    };

    template <template<typename> typename Func, typename T, typename... Ts>
    struct filter_tuple<Func, T, Ts...>
    {
        using type = std::conditional_t<Func<T>::value, typename filter_tuple<Func, Ts...>::type, decltype(std::tuple_cat(
                                            std::declval<std::tuple<T>>(),
                                            std::declval<typename filter_tuple<Func, Ts...>::type>()))>;
    };

    template <template<typename> typename Func, typename... Ts>
    struct filter_tuple<Func, std::tuple<Ts...>>
    {
        using type = typename filter_tuple<Func, Ts...>::type;
    };

    // func takes in the value at the tuple. Function should define integral constant.
    // accepts tuple or list of args
    template <template<typename> typename Func, typename... Ts>
    using filter_tuple_t = typename filter_tuple<Func, Ts...>::type;

    template<typename... Ts>
    struct filter_void
    {
        using type = typename filter_tuple<std::is_void, Ts...>::type;
    };
